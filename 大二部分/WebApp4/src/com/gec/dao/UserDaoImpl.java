...
public class UserDaoImpl implements UserDao{
	...
	public void updateUser(User user) throws SQLException {
		// TODO Auto-generated method stub
		String sql="update user set password=?,nickName=?,no=?,sex=?,roleId=? where id=?";
		Connection conn=JdbcUtils.getConnection();
		PreparedStatement psmt= conn.prepareStatement(sql);
		psmt.setString(1, user.getPassword());
		psmt.setString(2, user.getNickName());
		psmt.setString(3, user.getNo());
		psmt.setString(4, user.getSex());
		psmt.setString(5, user.getRoleId());
	...
	public User getUserById(String id) throws SQLException {
		String sql="select * from user u left join role r on r.id=u.role r on r.id=u.roldId where u.id=?";
		Connection conn=JdbcUtils.getConnection();
		PreparedStatement psmt= conn.prepareStatement(sql);
		psmt.setString(1, id);
		ResultSet rs=psmt.executeQuery();
		if(!rs.next()){
			throw new RuntimeException("查询不到该用户信息。");
		}
		User user=new User(rs);
		psmt.close();
		rs.close();
